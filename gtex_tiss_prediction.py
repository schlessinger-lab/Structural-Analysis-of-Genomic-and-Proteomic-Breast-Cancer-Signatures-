...
def predict_gtex(classifier, random_state, test_size, data_file_name, folder_out, tissue, write=True):
    ...
    if write:
        ...
        fpr, tpr, thresholds1 = roc_curve(y_test, y_test_predict_proba)
        precision, recall, thresholds2 = precision_recall_curve(y_test, y_test_predict_proba)
        new_out = ''
        for index in range(len(fpr)):
            new_out = new_out + str(fpr[index]) + ',' + str(tpr[index]) + '\n'

        new_prout = ''
        for index in range(len(precision)):
            new_prout = new_prout + str(recall[index]) + ',' + str(precision[index]) + '\n'
        f = open(folder_out + name + '_' + tissue + '_' + str(random_state) + '_prc.csv', 'w+')
        f.write(new_prout)
        f.close()
        f1 = open(folder_out + name + '_' + tissue + '_accuracy_pred.csv', 'a+')
        f1.write(str(random_state) + ','+str(acc)+','+str(aroc)+','+ str(f1_val)+','+str(precision_val)+','+str(recall_val)+'\n')
        f1.close()
        f2 = open(folder_out +name + '_' + tissue + '_' + str(random_state) +'_roc.csv', 'w+')
        f2.write(new_out)
        f2.close()
    return acc,aroc,f1_val,precision_val,recall_val

def run_crossfold(n, test_size, data_file_name, folder_out, write=True):
    ...
    for tiss in gtex_tiss_key:
        ...
        for i in range(n):
            print(i)
            predict_gtex(RandomForestClassifier(random_state=i), i, test_size, data_file_name, folder_out, tiss, write=write)

# run_crossfold(10, 0.2, 'norm_gtex_gn.csv', 'gtex_tiss_pred_out/', write=True)
run_crossfold(10, 0.2, 'norm_gtex_sages_gn.csv', 'gtex_tiss_pred_out/', write=True)
run_crossfold(10, 0.2, 'norm_gtex_sages.csv', 'gtex_tiss_pred_out/', write=True)
